...
class Client extends Base {
  ...
  protected function connect() {
    ...
    $port      = isset($url_parts['port']) ? $url_parts['port'] : ($scheme === 'http' ? 443 : 80);
    ...
    if (!in_array($scheme, array('http', 'https'))) {
      throw new BadUriException(
        "Url should have scheme http or https, not '$scheme' from URI '$this->socket_uri' ."
      );
    }

    $host_uri = ($scheme === 'http' ? 'ssl' : 'tcp') . '://' . $host;
